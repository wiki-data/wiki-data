...
class UploadLocalDirectory {
	...
	function execute() {
		...
		if (empty($this->_available_files)) {
			$folders = $this->_subfolders;
			$wgOut->addWikitext(count($folders).'KVA KVA'.$this->_directory);
			foreach ($folders as $folder) {
				$wgOut->addWikitext('<a href="/Special:UploadLocal?sub='.$folder.">".basename($folder)."</a>");
			}
	...
	function showForm() {
		...
		$html .= '<tbody>'.
			...
			'<tr><th style="text-align:right;">/search/ '.wfMsg('uploadlocal_dest_file_search').'</th>'.
			'<td><input type="text" style="width:100%;" name="wpDestFilesSearch" /></td></tr>'.
			
			'<tr><th style="text-align:right;">$replace '.wfMsg('uploadlocal_dest_file_replace').'</th>'.
			'<td><input type="text" style="width:100%;" name="wpDestFilesReplace" />
	...
	function getFilenamesOfFilesInDirectory($directory) {
		...
		while (($file = readdir($dh)) !== false) {
			...
			if (is_dir($directory . '/' . $file)) {
				$this->_subfolders[] = $directory . '/' . $file;
				continue;
			}
	...
	function processUploads( $request, $user ) {
		...
		$r_dest_files_search   = $request->getVal('wpDestFilesSearch', '');
		$r_dest_files_replace	= $request->getVal('wpDestFilesReplace', '');
		...
		foreach ($r_files as $i => $file) {
			...
			if ($r_dest_files_search && $r_dest_files_replace) {
				$r_dest_file = str_replace($r_dest_files_search,$r_dest_files_replace,$r_dest_file);
			}
